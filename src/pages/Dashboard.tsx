...
import { Brain, Clock, PlusCircle, Target, Flame, TrendingUp, Zap, AlertTriangle, CheckCircle } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
    ...
    const urgencyColor = displayRec?.urgency === 'high' ? 'text-red-400'
        : displayRec?.urgency === 'low' ? 'text-brand-primary' : 'text-brand-accent';
    const UrgencyIcon = displayRec?.urgency === 'high' ? AlertTriangle
                ...
                <div className="retro-panel p-6 lg:col-span-2 relative overflow-hidden border-brand-accent/30">
                    ...
                    <div className="flex justify-between items-start mb-4">
                        ...
                        {displayRec && !isLoadingAI && (
                            <span className={`text-xs font-mono flex items-center gap-1 ${urgencyColor}`}>
                                <UrgencyIcon size={12} /> {displayRec.urgency} priority
                            </span>
                        )}
                    </div>
                    ...
                    {displayRec ? (
                        <div className="space-y-4">
                            <div className="flex items-start gap-3">
                                <UrgencyIcon size={18} className={`${urgencyColor} mt-0.5 flex-shrink-0`} />
                                <div>
                                    <div className="text-brand-primary font-mono font-bold text-base">{displayRec.topic}</div>
                                    <div className="text-brand-secondary text-sm mt-1 leading-relaxed">{displayRec.reason}</div>
                                </div>
                            </div>
                            {displayRec.coachingNote && (
                                <div className="bg-brand-bg/50 border border-brand-primary/20 rounded p-3">
                                    <div className="text-xs font-mono text-brand-secondary uppercase tracking-widest mb-1">Coach's Tip</div>
                                    <div className="text-brand-primary/80 text-sm">{displayRec.coachingNote}</div>
                                </div>
                            )}
                            <div className="flex items-center gap-2 pt-1 border-t border-brand-border/20">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <div className="text-brand-primary font-mono font-bold">{deterministicRec.topic}</div>
                                    <div className="text-brand-secondary text-sm mt-1">{deterministicRec.reason}</div>
                                </div>
                            </div>
                            <div className="text-xs font-mono text-brand-secondary/50">Deterministic engine · Groq loading...</div>
                        </div>
                    ) : (
                        <p className="text-brand-secondary font-mono text-sm">Loading recommendation...</p>
                    )}
                </div>
